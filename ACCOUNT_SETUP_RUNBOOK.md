...
- Nachträglich verbunden: YouTube `Systemfehler_nach_DIN`, Kanal-ID `UCBWRl7VXRdy0kcsoV7or7Uw`, Buffer-Channel-ID `6a7cf0c4b2d9d57743679762`; `isDisconnected=false`. Damit soll YouTube künftig über Buffer statt über den direkten YouTube-OAuth-Adapter geplant/veröffentlicht werden.
- Die drei API-Schlüssel und Organisations-/Kanal-Metadaten liegen in Infisical unter `BUFFER_*`.
- Account 3 enthält nun zusätzlich den Buffer-YouTube-Kanal `Systemfehler_nach_DIN` (`UCBWRl7VXRdy0kcsoV7or7Uw`), Buffer-Channel-ID `6a7cf0c4b2d9d57743679762`; verifiziert `isDisconnected=false`. Die direkte YouTube-OAuth-Verbindung wird für diesen Publishing-Weg nicht benötigt.
