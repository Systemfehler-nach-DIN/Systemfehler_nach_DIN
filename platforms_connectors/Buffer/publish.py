...
def _input(payload: dict[str, Any], target: dict[str, Any]) -> dict[str, Any]:
    ...
    due_at = target.get("due_at") or payload.get("due_at") or payload.get("scheduled_at") or os.getenv("BUFFER_DUE_AT")
    if due_at:
        value["dueAt"] = str(due_at)
    asset = media_url(payload)
    media_type = str(target.get("media_type") or payload.get("media_type") or "").lower()
    if service in {"youtube", "tiktok"} and not media_type:
        media_type = "video"
    if asset:
        value["assets"] = [{"video" if media_type == "video" else "image": {"url": asset}}]
    metadata: dict[str, Any] = {}
    if service == "instagram":
        if not asset:
            raise ConnectorError("Buffer Instagram posts require media_url")
        metadata["instagram"] = {"type": str(target.get("instagram_type") or "post"), "shouldShareToFeed": bool(target.get("should_share_to_feed", True))}
    elif service == "facebook":
        metadata["facebook"] = {"type": str(target.get("facebook_type") or "post")}
    elif service == "pinterest":
        board = str(target.get("board_service_id") or os.getenv("BUFFER_BOARD_SERVICE_ID", "")).strip()
        if not board:
            raise ConnectorError("Buffer Pinterest requires buffer.board_service_id from channel metadata")
        metadata["pinterest"] = {"boardServiceId": board, "title": str(target.get("title") or payload.get("title") or "")}
    elif service == "youtube":
        if not asset or media_type != "video":
            raise ConnectorError("Buffer YouTube posts require a video media_url")
        metadata["youtube"] = {"title": str(target.get("title") or payload.get("title") or ""), "categoryId": str(target.get("category_id") or "22"), "privacy": str(target.get("privacy") or "private")}
    if metadata:
...
def _key_for(target: dict[str, Any]) -> str:
    ...
    if account and account.isdigit():
        return os.getenv(f"BUFFER_API_KEY_ACCOUNT_{account}", "").strip() or env("BUFFER_API_KEY")
...
def _create(value: dict[str, Any], key: str) -> dict[str, Any]:
    ...
    """
    response = request_json(ENDPOINT, headers={"Authorization": f"Bearer {key}"}, data={"query": query, "variables": {"input": value}})
...
def publish(payload: dict[str, Any], *, dry_run: bool = True) -> dict[str, Any]:
    ...
    if dry_run:
        return {**dry_result("buffer", BACKEND, endpoint=ENDPOINT, payload=payload), "targets": values}
