...
def publish_video(
    ...
    *,
    ...
    if (
        any(
            os.getenv(k, "").lower() != "true"
            for k in ("ALLOW_REAL_POSTS", "TIKTOK_BROWSER_LIVE_APPROVED")
        )
        or os.getenv("PUBLISH_MODE", "DRY_RUN").upper() != "LIVE"
    ):
    ...
    script = "\n".join(
        [
            ...
            + json.dumps(
                ...
                "if(!e)return false; e.focus(); e.innerText="
            ...
            "print(json.dumps({'ok': True, 'url': info.get('url', ''), 'description_length': len("
    ...
    if result.returncode:
        raise TikTokError(
            ...
            or "TikTok-Publish fehlgeschlagen"
        )
    return {
        "ok": True,
        "mode": "LIVE",
