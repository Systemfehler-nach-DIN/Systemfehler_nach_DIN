...
- Supabase-Staging: Schema/CLI-Adapter für `social-staging` ist lokal implementiert und in Infisical dokumentiert. Der bekannte Supabase-Endpoint `https://supabase.delqhi.com` ist erreichbar, aber der Read-only-REST-Check meldet HTTP 404/PGRST205: `public.media_assets` existiert noch nicht. Die Migration wurde nicht remote angewendet, weil `sin-supabase` bei der SSH-Discovery einen Tailscale-Auth-Gate verlangt. Es wurde keine Remote-Mutation ausgeführt.
